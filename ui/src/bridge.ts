...
function parsePreviewState(value: string | null): AutopilotState {
	if (!value) {
		return demoState();
	}
	try {
		...
		// Browser preview storage is disposable; a malformed cache must not prevent
		// the companion from opening or change its explicit preview mode.
		return demoState();
	}
}
...
export async function loadAutopilotState(): Promise<LoadedAutopilot> {
	...
	if (!current.storage) {
		throw new Error("Autopilot storage is unavailable. Reload before editing.");
	}
	...
	if (stored) {
		// A malformed live record must not turn into the demo workspace.
		try {
			state = normalizeState(JSON.parse(stored), emptyState());
		} catch {
			state = emptyState();
		}
	}
...
export async function runAutopilotCycle(
	...
	if (!current?.agent) {
		throw new Error(
			"Ryu agent execution is not available on this host. The strategy pass was not run."
		);
	}
